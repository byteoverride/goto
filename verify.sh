...
if grep -q "^abs_test|${TEST_DIR_1}$" "$XDG_CONFIG_HOME/goto/config"; then
...
count=$(grep -c "^abs_test|" "$XDG_CONFIG_HOME/goto/config")
path=$(grep "^abs_test|" "$XDG_CONFIG_HOME/goto/config" | cut -d'|' -f2-)
...
if ! grep -q "^del_test_d|" "$XDG_CONFIG_HOME/goto/config"; then
...
if ! grep -q "^del_test_u|" "$XDG_CONFIG_HOME/goto/config"; then
...
if ! grep -q "^rename_src|" "$XDG_CONFIG_HOME/goto/config" && grep -q "^rename_dst|${TEST_DIR_1}$" "$XDG_CONFIG_HOME/goto/config"; then
...
if ! grep -q "^cleanup_bad|" "$XDG_CONFIG_HOME/goto/config" && grep -q "^cleanup_good|" "$XDG_CONFIG_HOME/goto/config"; then
...
if grep -q "^import_one|" "$XDG_CONFIG_HOME/goto/config" && grep -q "^import_two|" "$XDG_CONFIG_HOME/goto/config"; then
...
printf 'notail|%s' "$TEST_DIR_1" > "$XDG_CONFIG_HOME/goto/config"
output=$($GOTO_BIN -l 2>&1)
...
printf 'notail|%s\n' "$TEST_DIR_1" > "$XDG_CONFIG_HOME/goto/config"
